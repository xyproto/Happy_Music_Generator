...
# These songs have already been converted from midi to msgpack
songs = get_songs('Happy_Music_Midi')
...
with tf.Session() as sess:
    ...
    for epoch in tqdm(list(range(num_epochs))):
        for song in songs:
            ...
            song = np.array(song)
            cutoff_point = int(np.floor(song.shape[0] / num_timesteps).tolist()) * num_timesteps
            song = song[:cutoff_point]
            a = int(song.shape[0] / num_timesteps)
            b = song.shape[1] * num_timesteps
            song = np.reshape(song, [a, b])
